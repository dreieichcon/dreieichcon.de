...
{
    name: "twitch",
    href: "https://www.twitch.tv/dreieichconworld",
...
window.getSocials = function () {
    return new Promise(
        (resolve, reject) => {
            axios.get(buildLink("/api/socials.php"))
                .then(result => {
                    if (result.data.socials) resolve(result.data.socials)
                    reject("Could not load socials from endpoint. Reverting to fallback value");
                })
...
window.getGlobals = function () {
    return new Promise(
        (resolve, reject) => {
            axios.get(buildLink("/api/globals.php"))
                .then(result => {
                    resolve(result.data)
                })
                .catch(error => reject(error))
